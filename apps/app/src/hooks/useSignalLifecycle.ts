  ...
  const analyzeSignal = useCallback(async (signalId: string, customModel?: string) => {
    ...
    if (!activeStrategy && signal.traderId) {
      ...
      try {
        ...
        console.error('Error fetching trader for signal:', signalId, error);
        return;
      }
    }
    ...
    try {
      ...
      const analysisPromise = analyzeSignal(nextSignalId)
        .finally(() => {
          ...
          // Just trigger queue check after a delay
          // The main processAnalysisQueue will handle the processing flag
