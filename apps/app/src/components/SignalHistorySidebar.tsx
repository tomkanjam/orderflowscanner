...
export function SignalHistorySidebar({ signal: initialSignal, onClose, tickers, traders }: SignalHistorySidebarProps) {
  ...
  return (
    <div className="h-full w-full bg-[var(--nt-bg-primary)] flex flex-col">
      {/* Header */}
      <div className="p-4 border-b border-[var(--nt-border-default)]">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold text-[var(--nt-text-primary)]">Signal History</h2>
          <button
            onClick={onClose}
            className="text-[var(--nt-text-muted)] hover:text-[var(--nt-text-primary)] transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-xl font-bold text-[var(--nt-text-primary)]">{signal.symbol}</span>
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div>
              <span className="text-[var(--nt-text-muted)]">Entry Price:</span>
              <span className="ml-2 font-mono">${signal.initialPrice.toFixed(4)}</span>
            </div>
            <div>
              <span className="text-[var(--nt-text-muted)]">Current:</span>
              <span className="ml-2 font-mono">${ticker?.c ? parseFloat(ticker.c).toFixed(4) : '---'}</span>
            </div>
            <div>
              <span className="text-[var(--nt-text-muted)]">Trader:</span>
              <span className="ml-2">{trader?.name || 'Unknown'}</span>
            </div>
            <div>
              <span className="text-[var(--nt-text-muted)]">Created:</span>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <h3 className="text-sm font-semibold text-[var(--nt-text-secondary)] mb-4 flex items-center gap-2">
          ...
          <div className="space-y-4">
            {signal.analysisHistory.slice().reverse().map((analysis, idx) => {
              ...
              return (
                <div 
                  key={idx} 
                  className={`relative pl-8 pb-4 ${idx < signal.analysisHistory!.length - 1 ? 'border-l-2 border-[var(--nt-border-default)]' : ''}`}
                  ...
                  <div className={`bg-[var(--nt-bg-secondary)] rounded-lg p-4 ${
                    ...
                    <div className="mb-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-[var(--nt-text-muted)]">Confidence</span>
                        <span className="text-xs font-medium">{Math.round(analysis.confidence * 100)}%</span>
                      </div>
                      <div className="w-full bg-[var(--nt-bg-hover)] rounded-full h-1.5">
                        <div 
                    ...
                    {/* Reasoning */}
                    <div className="space-y-2">
                      <p className="text-sm text-[var(--nt-text-secondary)] leading-relaxed">
                        {analysis.reasoning}
                      </p>
                      
                      {/* Trade Plan */}
                      {analysis.tradePlan && (
                        <div className="mt-3 p-3 bg-[var(--nt-bg-hover)] rounded-lg">
                          <div className="flex items-center gap-2 mb-2">
                            <Target className="w-4 h-4 text-[var(--nt-text-muted)]" />
                            <span className="text-xs font-medium text-[var(--nt-text-secondary)]">Trade Plan</span>
                          </div>
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            ...
                            {analysis.tradePlan.notes && (
                              <div className="col-span-2">
                                <span className="text-[var(--nt-text-muted)]">Notes:</span>
                                <p className="mt-1 text-[var(--nt-text-secondary)]">{analysis.tradePlan.notes}</p>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      {analysis.keyLevels && (
                        <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
                          {analysis.keyLevels.support && analysis.keyLevels.support.length > 0 && (
                            <div>
                              <span className="text-[var(--nt-text-muted)]">Support:</span>
                          ...
                          {analysis.keyLevels.resistance && analysis.keyLevels.resistance.length > 0 && (
                            <div>
                              <span className="text-[var(--nt-text-muted)]">Resistance:</span>
          ...
          <div className="mt-8">
            ...
            <div className="space-y-2">
              {signal.monitoringUpdates.slice(-10).reverse().map((update, idx) => (
                <div key={idx} className="flex items-center justify-between p-2 bg-[var(--nt-bg-secondary)] rounded text-xs">
