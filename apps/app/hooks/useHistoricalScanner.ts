          ...
          switch (response.type) {
            ...
            case "complete":
              if (!abortControllerRef.current?.signal.aborted) {
                ...
                const sortedSignals = response.signals.sort(
                  (a: HistoricalSignal, b: HistoricalSignal) =>
                    a.klineTimestamp - b.klineTimestamp,
                );

                // Apply deduplication logic
                const deduplicatedSignals: HistoricalSignal[] = [];
                const symbolLastSignal = new Map<
                  string,
                  { signalIndex: number; klineTimestamp: number }
                >();
                ...
                for (const signal of sortedSignals) {
                  ...
                  if (lastSignalInfo) {
                    const timeDiff = Math.abs(
                      signal.klineTimestamp - lastSignalInfo.klineTimestamp,
                    );
                  ...
                  if (!lastSignalInfo || barDistance >= signalDedupeThreshold) {
                    ...
                    const newIndex =
                      deduplicatedSignals.push(enhancedSignal) - 1;
                    symbolLastSignal.set(signal.symbol, {
                      signalIndex: newIndex,
                      klineTimestamp: signal.klineTimestamp,
                    });
                  } else {
                    ...
                    if (existingSignalIndex < deduplicatedSignals.length) {
                      deduplicatedSignals[existingSignalIndex].count =
                        (deduplicatedSignals[existingSignalIndex].count || 1) +
                        1;
                    }
                  }
                }
                ...
                const finalSignals = deduplicatedSignals.sort(
                  (a, b) => b.klineTimestamp - a.klineTimestamp,
                );
