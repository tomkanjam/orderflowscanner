...
### Ready for Testing
All code changes complete. Implementation follows the planned architecture (Option 1 - store in existing JSONB column).
...
- [ ] Create new signal and verify klines stored in `indicator_data`
- [ ] Verify JSON structure: `{"rsi_14": [...], "klines": [...]}`
- [ ] Verify 150 klines stored (not more, not less)
- [ ] Frontend displays chart without fetching from Binance
- [ ] Historical signals show correct candlestick data
- [ ] Chart rendering performance is acceptable
- [ ] Database query performance unchanged
- [ ] Storage size per signal is reasonable (~8-10KB)

### Rollout Plan

1. **Phase 1:** Deploy backend changes (executor.go)
2. **Phase 2:** Deploy frontend changes (interfaces, ChartDisplay, App)
3. **Phase 3:** Monitor storage impact and query performance
4. **Phase 4:** Optional: Backfill existing signals (if needed)
