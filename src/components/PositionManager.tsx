    ...
    <div className="bg-[#0d1421] border border-[#1a2332] rounded-lg p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-lg font-semibold text-[#8efbba]">
      ...
      <div className="bg-[#1a2332] rounded-lg p-4 mb-4">
        <div className="text-center">
          <p className="text-sm text-[#64748b] mb-1">Unrealized P&L</p>
      ...
      <div className="space-y-3 mb-4">
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            ...
            <p className="text-[#e2e8f0] font-mono">${entryPrice.toFixed(4)}</p>
          </div>
          <div>
            <p className="text-[#64748b]">Current Price</p>
            <p className="text-[#e2e8f0] font-mono">${currentPrice.toFixed(4)}</p>
          </div>
          <div>
            <p className="text-[#64748b]">Position Size</p>
            <p className="text-[#e2e8f0] font-mono">${trade.positionSize?.toFixed(2)} USDT</p>
          </div>
          <div>
        ...
        {isEditing ? (
          <div className="space-y-2 pt-2 border-t border-[#2a3441]">
            <div>
              <label className="text-xs text-[#64748b]">Stop Loss</label>
              <input
                type="number"
                value={editStopLoss}
                onChange={(e) => setEditStopLoss(e.target.value)}
                className="w-full px-2 py-1 bg-[#1a2332] border border-[#2a3441] rounded text-[#e2e8f0] text-sm"
              />
            </div>
            <div>
              <label className="text-xs text-[#64748b]">Take Profit 1</label>
              <input
                type="number"
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-1 bg-[#8efbba] text-[#0d1421] rounded text-sm hover:bg-[#7ce3a8]"
              >
                Save
              </button>
              <button
                onClick={() => setIsEditing(false)}
                className="px-3 py-1 bg-[#2a3441] text-[#e2e8f0] rounded text-sm hover:bg-[#3a4451]"
              >
                Cancel
              </button>
            </div>
            ...
            {trade.stopLoss && (
              <div className="flex justify-between text-sm">
                ...
                <div className="text-right">
                  ...
                  <span className="text-xs text-[#64748b] ml-2">({distanceToSL.toFixed(1)}% away)</span>
                </div>
              </div>
            )}
            {trade.takeProfit?.[0] && (
              <div className="flex justify-between text-sm">
                <span className="text-[#64748b]">Take Profit 1</span>
                <div className="text-right">
                  <span className="text-green-400 font-mono">${trade.takeProfit[0].toFixed(4)}</span>
                  <span className="text-xs text-[#64748b] ml-2">({distanceToTP.toFixed(1)}% away)</span>
                </div>
              </div>
            )}
          </div>
      ...
      {!isClosing ? (
        ...
        <div className="space-y-3">
          <textarea
            ...
            className="w-full px-3 py-2 bg-[#1a2332] border border-[#2a3441] rounded text-[#e2e8f0] text-sm resize-none h-20"
          />
          <div className="flex gap-2">
            <button
              onClick={() => setIsClosing(false)}
              className="flex-1 px-3 py-2 bg-[#2a3441] text-[#e2e8f0] rounded hover:bg-[#3a4451] transition-colors"
            >
              Cancel
            </button>
            <button
