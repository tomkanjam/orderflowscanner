...
import {
  ...
  filterSignals,
} from "../components/demo/mockSignals";
...
export const StyleGuide: React.FC = () => {
  ...
  const filteredSignals = useMemo(
    () => filterSignals(mockSignals, filterQuery),
    [filterQuery],
  );
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <aside className="w-[360px] border-r border-border bg-background h-screen sticky top-0 flex flex-col">
        ...
        <div className="px-4 pt-4 pb-2">
          <button className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-all font-medium">
                  ...
                  <div className="space-y-2">
                    {signals.map((signal) => (
                      <ExpandableSignalCard
                        ...
                        onToggleExpand={() =>
                          setExpandedCardId(
                            expandedCardId === signal.id ? null : signal.id,
                          )
                        }
                      />
                    ))}
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              A comprehensive style guide showcasing shadcn/ui components with
              Supabase styling
            </p>
          </div>
        ...
        <section className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Features: Tabs (Built-in, Personal, Favorites), real-time
                filter, category subsections, expandable cards
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2">
                {activeTab === "builtin" && (
                  <>
                    {Object.entries(groupedBuiltIn).map(
                      ([category, signals]) => (
                        <div key={category}>
                          <CategoryHeader
                            category={category}
              ...
              {filterQuery && getCurrentSignals().length > 0 && (
                <div className="text-xs text-muted-foreground text-center pt-2 border-t border-border">
                  Showing {getCurrentSignals().length} of{" "}
                  {
                    mockSignals.filter((s) =>
                      activeTab === "builtin"
                        ? s.isBuiltIn
                        : activeTab === "personal"
                          ? !s.isBuiltIn
