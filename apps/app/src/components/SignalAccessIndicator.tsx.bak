...
export function SignalAccessIndicator({ 
  ...
  return (
    <div className={`inline-flex items-center gap-1 ${className}`}>
      {isLocked && showLock && (
        <Lock className="w-3 h-3 text-[var(--tm-text-muted)]" />
      )}
      ...
      {signal.difficulty && (
        <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-[var(--tm-bg-tertiary)] text-[var(--tm-text-secondary)]">
          {signal.difficulty}
        </span>
      )}
    </div>
