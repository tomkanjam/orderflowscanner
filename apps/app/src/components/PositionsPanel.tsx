    ...
    <div className="tm-card p-3">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-semibold text-[var(--tm-text-primary)]">Positions & P&L</h3>
          {tradingMode === 'demo' ? (
      ...
      {/* Balance Summary */}
      <div className="mb-3 p-2 bg-[var(--tm-bg-hover)] rounded-md">
        <div className="flex items-center justify-between text-sm">
          <span className="text-[var(--tm-text-muted)]">Balance</span>
          <span className="font-mono font-semibold">
            ${usdtBalance?.total.toFixed(2) || '0.00'} USDT
          </span>
        </div>
        <div className="flex items-center justify-between text-sm mt-1">
          <span className="text-[var(--tm-text-muted)]">Available</span>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="p-2 bg-[var(--tm-bg-hover)] rounded-md">
          <div className="text-xs text-[var(--tm-text-muted)]">Unrealized P&L</div>
        ...
        <div className="p-2 bg-[var(--tm-bg-hover)] rounded-md">
          <div className="text-xs text-[var(--tm-text-muted)]">Realized P&L</div>
          <div className={`text-sm font-mono font-semibold ${
      ...
      {demoStats && (
        <div className="mb-3 p-2 bg-blue-500/10 border border-blue-500/30 rounded-md text-xs">
          <div className="grid grid-cols-2 gap-2">
            <div>
              <span className="text-[var(--tm-text-muted)]">Total Trades:</span>{' '}
              <span className="font-semibold">{demoStats.totalTrades}</span>
            </div>
            <div>
              <span className="text-[var(--tm-text-muted)]">Win Rate:</span>{' '}
      ...
      <div className="space-y-2 max-h-[400px] overflow-y-auto">
        {isLoading && positions.length === 0 ? (
          <div className="text-center py-4 text-[var(--tm-text-muted)] text-sm">
            Loading positions...
          </div>
        ) : positions.length === 0 ? (
          <div className="text-center py-4 text-[var(--tm-text-muted)] text-sm">
          ...
          positions.map(position => (
            <div
              key={position.id}
              className="p-2 bg-[var(--tm-bg-hover)] rounded-md space-y-1"
            >
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                ...
                <div>
                  <span className="text-[var(--tm-text-muted)]">Mark:</span>{' '}
              ...
              <div className="flex items-center justify-between text-xs">
                <div>
                  <span className="text-[var(--tm-text-muted)]">Unrealized:</span>{' '}
                  <span className={`font-mono ${
